...
#pragma multi_compile _ _MAIN_LIGHT_SHADOWS_CASCADE //Aunn: Junto con la línea anterior permite que haya sombras con niveles de detalles
#pragma multi_compile _ _SHADOWS_SOFT //Aunn: Permite el suavizado de sombras
#pragma multi_compile _ _ADDITIONAL_LIGHTS //Casi con total seguridad  esta línea y la siguiente permiten que las luces adicionales proyecten sombras, aunque todavía no funciona
...
void AdditionalLights_float(float3 SpecColor, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, out float3 Diffuse, out float3 Specular)
{
    ...
    for (int i = 0; i < pixelLightCount; ++i) // Aunn: Ajusta "i=1" para que salte la luz principal (que sería el "0" siempre y cuando esta sea la luz con mayor intensidad) si te está dando problemas.
    {
